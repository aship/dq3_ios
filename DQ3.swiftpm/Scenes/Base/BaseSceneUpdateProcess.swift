...
extension BaseScene {
    func processUpdate(
        ...
        if self.fieldMoveMode == .walk && self.ramiaDirection != .neutral {
        ...
        if self.fieldMoveMode == .walk || self.fieldMoveMode == .ship {
        ...
        if self.fieldMoveMode == .ship {
            ...
            if DataManager.adventureLog.dqSceneType == .field {
                isLanding = self.fieldScene?.checkLandingFromShip(
                    tileMapNode: tileMapNode,
                    newPositionX: newPositionX,
                    newPositionY: newPositionY)
            } else if DataManager.adventureLog.dqSceneType == .alefgard {
                isLanding = self.alefgardScene?.checkLandingFromShip(
                    tileMapNode: tileMapNode,
                    newPositionX: newPositionX,
                    newPositionY: newPositionY)
            }
            ...
            if isLanding! {
                ...
                if DataManager.adventureLog.dqSceneType == .field {
                    // 船を降りる処理
                    self.fieldScene?.processLandingFromShip(
                        characterNodes: &characterNodes,
                        tileMapNode: tileMapNode,
                        scale: scale)
                } else if DataManager.adventureLog.dqSceneType == .alefgard {
                    // 船を降りる処理
                    self.alefgardScene?.processLandingFromShip(
                        characterNodes: &characterNodes,
                        tileMapNode: tileMapNode,
                        scale: scale)
                }
