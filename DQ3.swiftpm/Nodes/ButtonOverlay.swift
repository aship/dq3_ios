...
class ButtonOverlay: SKNode {
    ...
    func buildPad(_ text: NSString) {
        let backgroundRect = CGRect(
            x: CGFloat(0), y: CGFloat(0), width: CGFloat(size.width), height: CGFloat(size.height))
        ...
        inner = SKShapeNode()
        inner.path = CGPath(
            ellipseIn: CGRect(x: 0, y: 0, width: innerSize.width, height: innerSize.height),
            transform: nil)
        inner.lineWidth = 1.0
    ...
    func updateForSizeChange() {
        ...
        let innerRect = CGRect(
            x: CGFloat(0), y: CGFloat(0), width: CGFloat(size.width / 3.0),
            height: CGFloat(size.height / 3.0))
        inner.path = CGPath(ellipseIn: innerRect, transform: nil)

        label!.position = CGPoint(
            x: size.width / 2.0 - label!.frame.size.width / 2.0,
            y: size.height / 2.0 - label!.frame.size.height / 2.0)
    }
