...
class PadOverlay: SKNode {
    ...
    func buildPad() {
        let backgroundRect = CGRect(x: CGFloat(0),
                                    y: CGFloat(0),
                                    width: CGFloat(size.width),
                                    height: CGFloat(size.height))
        background = SKShapeNode()
        background.path = CGPath(ellipseIn: backgroundRect,
                                 transform: nil )
        ...
        stick = SKShapeNode()
        stick.path = CGPath(ellipseIn: stickRect,
                            transform: nil)
        stick.lineWidth = 2.0
    ...
    func updateForSizeChange() {
        ...
        background.path = CGPath(ellipseIn: backgroundRect,
                                 transform: nil)
        let stickRect = CGRect(x: CGFloat(0),
                               y: CGFloat(0),
                               width: CGFloat(size.width / 3.0),
                               height: CGFloat(size.height / 3.0))
        stick.path = CGPath(ellipseIn: stickRect,
                            transform: nil)
    }
    ...
    func updateStickPosition() {
        ...
        stick.position = CGPoint(x: stickX,
                                 y: stickY)
    }
    
    func updateStickPosition(forTouchLocation location: CGPoint) {
        var l_vec = vector_float2(x: Float( location.x - startLocation.x),
                                  y: Float( location.y - startLocation.y))
        ...
        stickPosition = CGPoint(x: CGFloat(l_vec.x),
                                y: CGFloat(l_vec.y))
    }
