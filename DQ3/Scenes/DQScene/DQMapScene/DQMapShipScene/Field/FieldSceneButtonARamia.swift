...
extension FieldScene {
    func processButtonARamia(mapCommandWindowNode: inout MapCommandWindowNode,
        ...
        if scene.fieldMoveMode == .walk &&
            self.ramiaNode?.positionX == headNode.positionX &&
            self.ramiaNode?.positionY == headNode.positionY {
            self.processRideOnRamia(tileMapNode: tileMapNode,
                                    ramiaNode: ramiaNode,
            ...
            processButtonAOnRamia(ramiaNode: ramiaNode,
    ...
    private func processButtonAOnRamia(ramiaNode: CharacterNode,
        ...
        let canGetOffRamia = checkGetOffRamia(tileMapNode: tileMapNode,
                                              positionX: headNode.positionX,
                                              positionY: headNode.positionY)
        if canGetOffRamia {
            queueFollowDirections = []
            
            processGetOffRamia(tileMapNode: tileMapNode,
                               ramiaNode: ramiaNode,
                               fieldMoveMode: &scene.fieldMoveMode,
                               characterNodes: partyCharacterNodes,
                               scale: scale)
