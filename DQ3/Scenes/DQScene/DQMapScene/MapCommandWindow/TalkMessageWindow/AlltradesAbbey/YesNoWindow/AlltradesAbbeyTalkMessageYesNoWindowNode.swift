        ...
        if self.name == "minister" {
            if parentNode.ministerMessageFlag == .message3 {
                if self.dqYesNo == .yes {
                    ...
                    Task {
                        await parentNode.showMessage(string: text1,
                                                     line: 3,
                                                     withSe: true)
                        parentNode.moveLine()
            ...
            if parentNode.ministerMessageFlag == .ask_going_now {
                if self.dqYesNo == .yes {
                    ...
                    Task {
                        ...
                        await parentNode.showMessage(string: text2,
                                                     line: 3,
                                                     withSe: true)
...
func showAskGoingNow(windowNode: TalkMessageWindowNodeAlltradesAbbey) {
  ...
  //  let parent = windowNode.parent as! MapCommandWindowNode
    ...
    Task {
        await windowNode.showMessage(string: text1,
                                     line: 3,
                                     withSe: true)
        await windowNode.moveLine()
        
        await windowNode.showMessage(string: text2,
                                     line: 3,
                                     withSe: true)
        await windowNode.moveLine()
