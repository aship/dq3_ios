...
extension BattleScene {
    func asyncBattle(characterStatus: CharacterStatus,
                     battleActionStruct: BattleActionStruct,
                     completion: @escaping () -> Void) {
        print("# asyncBattle \(characterStatus.name) Start \(battleActionStruct.battleActionType.rawValue)")
        ...
        addMessageWindow(battleMessageWindowNode: &self.battleMessageWindowNode,
                         scene: self.scene,
        ...
        if battleActionType == .attack {
            ...
            let runDamageAction = SKAction.run({
                ...
                self.showDamageMessage(monsterNode: monsterNode,
            ...
            let runShowDamageMessage = SKAction.run({
                if monsterNode.hp <= 0 {
                    ...
                    self.defeatMonster(monsterNode: monsterNode,
                                       monsterIndexAlphabet: monsterIndexAlphabet)
            ...
            let allActions = SKAction.sequence([actionWait1sec,
                                                runDamageAction,
                                                actionWait1sec,
