...
extension AdventureLogInputNameWindowNode {
    ...
    private func getDiff(direction: Direction,
                         diffX: inout CGFloat,
                         diffY: inout CGFloat,
        ...
        if trianglePositionX == 4 &&
            direction == .right {
            // 左ブロックから右ブロックに飛ぶ
            diffX += 8
        }
        
        if trianglePositionX == 5 &&
            direction == .left {
            // 右ブロックから左ブロックに飛ぶ
            diffX -= 8
        }
        
        if trianglePositionX == 6 &&
            trianglePositionY == 1 {
            if direction == .down {
                // "み"の時に、下にちょっと飛ぶ
                diffY -= 16
